  ...
  return (
    <section id="garages" className="bg-white py-20 text-gray-900 border-t border-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-2xl p-5 mb-8 grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
        ...
        {filteredGarages.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredGarages.map((garage) => {
              return (
                <div
                  key={garage.id}
                  className="bg-white border border-gray-200/80 hover:border-signal/50 hover:shadow-lg rounded-xl p-6 transition-all duration-300 flex flex-col justify-between group"
                >
                  <div>
                    ...
                    <div className="flex justify-between items-start gap-4">
                      ...
                      {garage.verified && (
                        <div className="flex items-center space-x-1.5 bg-teal-50 border border-teal-200 text-teal-800 px-2 py-1 rounded text-2xs font-mono font-bold tracking-widest shrink-0">
                    ...
                    {/* Location */}
                    <div className="flex items-center space-x-2 text-gray-500 text-xs mt-2.5">
                      <MapPin className="h-3.5 w-3.5 text-signal" />
                      <span className="font-sans">{garage.location}</span>
                    </div>

                    {/* Rating & Reviews */}
                    <div className="flex items-center space-x-2 mt-3 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-lg w-fit">
                      <Star className="h-3.5 w-3.5 fill-amber-400 text-amber-400" />
                      <span className="font-mono text-sm font-bold text-gray-800">{garage.rating.toFixed(1)}</span>
                      <span className="text-gray-300 font-sans text-xs">|</span>
                      <span className="font-mono text-xs text-gray-500">{garage.reviews} verified reviews</span>
                    </div>
                        ...
                        <span
                          ...
                          className={`font-mono text-2xs px-2.5 py-1 rounded border transition-colors cursor-pointer ${
                            selectedServiceFilter.toLowerCase() === tag.toLowerCase()
                              ? "bg-signal/10 border-signal text-signal font-semibold"
                  ...
                  <div className="mt-8 pt-4 border-t border-gray-100 flex items-center justify-between">
                    <div className="text-left">
                      <span className="block text-3xs text-gray-400 uppercase tracking-wider font-mono">Availability</span>
                      <span className="text-xs text-teal-600 font-medium font-sans flex items-center space-x-1.5 mt-0.5">
                    ...
                    <button
                      ...
                      className="bg-signal hover:bg-signal/90 text-white font-display font-bold text-xs uppercase tracking-wider px-5 py-3 rounded-lg transition-all duration-200 transform active:scale-95 cursor-pointer shadow-md shadow-signal/10"
                    >
                      Book Appointment
                    </button>
                  </div>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-12 text-center max-w-xl mx-auto">
            ...
            <button
              ...
              className="mt-6 bg-white border border-gray-300 hover:border-gray-800 text-gray-700 font-display text-xs uppercase tracking-wider px-4 py-2.5 rounded-lg transition-colors"
            >
              Reset All Filters
            </button>
          </div>
